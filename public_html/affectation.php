...
$select7 = mysqli_query($link,"select id from etudiant where classement='1' and spécialité ='isi'");
$data7 = mysqli_fetch_assoc($select7);

$select9 = mysqli_query($link,"select * from etudiant where spécialité ='isi' ORDER BY classement");
...
$update = mysqli_query($link,"update etudiant set sujet_id ='$sujet_id8' where classement='1' and spécialité ='isi' ");
$update3 = mysqli_query($link,"update sujet set etat ='affecté' where id ='$sujet_id8' ");

for ($i=2;$i<=$rowcount;$i++) {
  for($p=1;$p<=3;$p++) {

    $select = mysqli_query($link,"select id from etudiant where classement=$i and spécialité ='isi'");
    ...
    for ($c=1;$c<$i;$c++) {
                     ...
                     $select3 = mysqli_query($link,"select sujet_id from etudiant where classement=$c and spécialité ='isi'");
                     ...
                     $sujet_id3=$data3['sujet_id'];
                      ...
                      if($sujet_id2!=$sujet_id3){
                        $update2 = mysqli_query($link,"update etudiant set sujet_id ='$sujet_id2' where classement='$i' and spécialité ='isi' ");
  ...
  for ($j=2;$j<=$rowcount;$j++) {
    $select10 = mysqli_query($link,"select sujet_id from etudiant where classement='$j' and spécialité ='isi'");
    $data10 = mysqli_fetch_assoc($select10);
    $sujet_id10=$data10['sujet_id']; 
     if(is_null($sujet_id10)){

      $select11 = mysqli_query($link,"select id from sujet where etat='validé' and spécialité ='isi' ");
...
/*       $id = array_keys($data11['id']);
      ...
      $update11 = mysqli_query($link,"update etudiant set sujet_id ='$sujet_id11' where classement='$j' and spécialité ='isi' ");
