...
if(!empty($_REQUEST['email']) AND !empty($_REQUEST['password'])){
    ...
    $select = mysqli_query($link, "SELECT * FROM enseignant WHERE email = '".$email."' AND status='ACTIVE'");
    $result = mysqli_fetch_assoc($select) ;
    if(mysqli_num_rows($select)) {

        $select = mysqli_query($link, "SELECT * FROM enseignant WHERE password = '".$password."'");
             if(mysqli_num_rows($select)) {
