...
$specialite = mysqli_real_escape_string($link, $_REQUEST['spécialité']);
...
if(!empty($_REQUEST['nom']) AND !empty($_REQUEST['prenom']) AND !empty($_REQUEST['email1']) AND !empty($_REQUEST['email2']) AND !empty($_REQUEST['password1']) AND !empty($_REQUEST['password2']) AND !empty($_REQUEST['spécialité'])){

if($email1==$email2){
    if(filter_var($email1, FILTER_VALIDATE_EMAIL)) {
   
$select = mysqli_query($link, "SELECT * FROM etudiant WHERE email = '".$_REQUEST['email1']."'");
if(mysqli_num_rows($select)) {
    $result='<div class="alert alert-danger">Cette adresse email est déjà utilisé</div>';
    header("location: /univ-mosta.tech/public_html/etudiant/inscription.php?result=".urlencode($result));
   // exit('Cette adresse email est déjà utilisé');
}else{

if($password1==$password2){
// Prepare an insert statement
$sql = "INSERT INTO etudiant (nom,prenom,email,password,filiere_id,status,token) VALUES (?,?,?,?,?,?,?)";

if($stmt = mysqli_prepare($link, $sql)){
    // Bind variables to the prepared statement as parameters
    mysqli_stmt_bind_param($stmt, "ssssiss", $nom, $prenom,$email1,$password1,$specialite,$status,$token);
    ...
    if(mysqli_stmt_execute($stmt)){
        ...
        $url = 'http://'.$_SERVER['SERVER_NAME'].'/univ-mosta.tech/public_html/etudiant/verify.php?id='.$last_id.'&token='.$token;
